...
class Maya_LightRebuild(form_class,base_class):
    ...
    def Light_Import_Fn(self):
        ...
        for lightName in Light_Json['LightData']:
            lightNameData = Light_Json['LightData'][lightNameData]

            if lightNameData['Type'] == "pointLight":
                light = cmds.pointLight(n=lightNameData['Name'])
            elif lightNameData['Type'] == "spotLight":
                light = cmds.spotLight(n=lightNameData['Name'])
                ...
                light = cmds.shadingNode('areaLight',asLight=1, n=lightName)
                lightParent = cmds.listRelatives( lightName, p=1 )[0]
                cmds.rename(lightParent,lightNameData['Name'])
            elif lightNameData['Type'] == "directionalLight":
                light = cmds.directionalLight(n=lightNameData['Name'])
            elif lightNameData['Type'] == "aiSkyDomeLight":
                mtoa.utils.createLocatorWithName("aiSkyDomeLight",lightNameData['Name'],asLight=True)
            elif lightNameData['Type'] == "aiAreaLight1":
                mtoa.utils.createLocatorWithName("aiAreaLight",lightNameData['Name'],asLight=True)

            R = lightNameData['color']['R']
            G = lightNameData['color']['G']
            B = lightNameData['color']['B']
            cmds.setAttr(light+".color",R,G,B,type="double3")

            cmds.setAttr(light+".intensity",lightNameData['Intensity'])
            cmds.setAttr(light+".aiExposure",lightNameData['Exposure'])

            tx = lightNameData['Translate']['tx']
            ty = lightNameData['Translate']['ty']
            tz = lightNameData['Translate']['tz']
            cmds.setAttr(lightNameData['Name']+".tx",tx)
            cmds.setAttr(lightNameData['Name']+".ty",ty)
            cmds.setAttr(lightNameData['Name']+".tz",tz)

            rx = lightNameData['Rotate']['rx']
            ry = lightNameData['Rotate']['ry']
            rz = lightNameData['Rotate']['rz']
            cmds.setAttr(lightNameData['Name']+".rx",rx)
            cmds.setAttr(lightNameData['Name']+".ry",ry)
            cmds.setAttr(lightNameData['Name']+".rz",rz)

            sx = lightNameData['Scale']['sx']
            sy = lightNameData['Scale']['sy']
            sz = lightNameData['Scale']['sz']
            cmds.setAttr(lightNameData['Name']+".sx",sx)
            cmds.setAttr(lightNameData['Name']+".sy",sy)
            cmds.setAttr(lightNameData['Name']+".sz",sz)
