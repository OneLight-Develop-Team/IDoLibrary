...
"""
    ...
    def Process_UI_Data(self):
        ...
        try:
            ...
            with open(UI_Location, 'r' , encoding='UTF-8') as f:
                uiFile = f.read()
...
class UI_Interface(base_class,form_class):
    def __init__(self,dock="dock"):
        ...
        ptr = self.Dock_Win_Management(title=u"%s")
...
""".format(QLineEdit)
        ...
        for QPlainText in QPlainTextEditList:
            SaveJsonFun += "    " + "GUI_STATE['{0}'] = self.{0}.text() if len(self.{0}.text())>0 else \"\"".format(QPlainText) + "\n"
...
"""
        ...
        # 处理 Json 函数的缩进
        reg = r'.*\n'
        ToggleList = re.findall(re.compile(reg),SaveJsonFun)
        TabToggleList = ""
        for Toggle in ToggleList:
            TabToggleList += "    " + Toggle

        SaveJsonFun = TabToggleList

        # 处理 读取Json 按钮缩进
        reg = r'.*\n'
        ToggleList = re.findall(re.compile(reg),LoadJsonFun)
        TabToggleList = ""
        for Toggle in ToggleList:
            TabToggleList += "    " + Toggle
        LoadJsonFun = TabToggleList

        Result += Toggle_Button_Click
        Result += Load_JSON_BTN
        Result += Save_JSON_BTN
        if self.Maya_RB.isChecked():
        ...
        Result += Load_JSON_Fun
        Result += Save_JSON_Fun
...
"""
        Init_Class = """
from {0}_ui import UI_Interface
class {0}(UI_Interface):
    def __init__(self,dock="dock"):
        super({0},self).__init__(dock=dock)
